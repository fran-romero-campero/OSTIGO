...
# Get the order of rows based on the first column; first, we need to create the vector
order <- order(ostta_v3_go_annotation[, 1])
# Reorder the rows of the dataframe according to the order vector 
ostta_v3_go_annotation <- ostta_v3_go_annotation[order, ]

# Observe
# Identify duplicate rows in columns 1 and 2 (genes and GO)
duplicate_rows <- duplicated(ostta_v3_go_annotation[, c(1, 2)]) | duplicated(ostta_v3_go_annotation[, c(1, 2)], fromLast = TRUE)
# Select only the duplicate rows
duplicate_data <- ostta_v3_go_annotation[duplicate_rows, ]
# View the result
print(duplicate_data)

# Remove
# Remove duplicate rows in columns 1 and 2 (genes and GO), leaving one copy
ostta_v3_go_annotation <- unique(ostta_v3_go_annotation[, c(1, 2, 3)])
# Print the result (optional)
print(ostta_v3_go_annotation)

# Save the ostta_v2_go_annotation dataframe to a TSV file without quotes
write.table(ostta_v3_go_annotation, "./annotation/ostta_v3_go_annotation.tsv", sep = "\t", quote = FALSE, row.names = FALSE)

# Is there any improvement ostta_v2_go_annotation compared to version 1?
ostta.annoation.v3 <- read.table(file="./annotation/ostta_v3_go_annotation.tsv",header=T,sep="\t")

head(ostta.annoation.v3)
length(unique(ostta.annoation.v3$GID))
length(unique(ostta.annoation.v3$GID))/7668 #we see that the annotation has improved around 4%
...
makeOrgPackage(go=ostta.annoation.v3,
               version = "3.1",
...
install.packages("./org.Otauriv3.eg.db/", repos=NULL, type = "source")
library(org.Otauriv3.eg.db)
