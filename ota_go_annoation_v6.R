## Load v1 GO annotation for Ostreococcus tauri
...
ostta_v2_go_annotation <- ostta.annoation.v2
## Generating v4

## Lipid Genes 2025
lipid.metabolism.genes <- read.table(file="manual_expansion/core_lipid_genes_2025.tsv",header=F,as.is = T)[[1]]
length(lipid.metabolism.genes)
lipid.metabolic.process.go <- "GO:0006629" #https://amigo.geneontology.org/amigo/term/GO:0006629

lipid.genes.df <- data.frame(lipid.metabolism.genes,
           rep(lipid.metabolic.process.go,length(lipid.metabolism.genes)),
           rep("ISS",length(lipid.metabolism.genes)))

colnames(lipid.genes.df) <- colnames(ostta_v2_go_annotation)

## Genes involved in chromatin state
chromatin.genes <- read.table(file="manual_expansion/chromatin_genes.tsv",header=F,as.is=T,sep="\t")
head(chromatin.genes)

chromatin.genes.df <- data.frame(chromatin.genes$V1,
                                 chromatin.genes$V2,
                                 rep("ISS",nrow(chromatin.genes)))
colnames(chromatin.genes.df) <- colnames(ostta_v2_go_annotation)

## Genes involved in transcription regulation (TFs)
tfs.genes <- read.table(file="manual_expansion/tf_genes_2025.tsv",header=T,as.is = T,sep="\t")[[1]]
length(tfs.genes)
tfs.process.go <- "GO:0006351" #https://www.ebi.ac.uk/QuickGO/GTerm?id=GO:0006351

tfs.genes.df <- data.frame(tfs.genes,
                             rep(tfs.process.go,length(tfs.genes)),
                             rep("ISS",length(tfs.genes)))

colnames(tfs.genes.df) <- colnames(ostta_v2_go_annotation)


## Genes involved in transcription regulation (TFs)
trs.genes <- read.table(file="manual_expansion/tr_genes_2025.tsv",header=T,as.is = T,sep="\t")[[1]]
length(trs.genes)
trs.mf.go <- "GO:0003712" #https://amigo.geneontology.org/amigo/term/GO:0003712

trs.genes.df <- data.frame(trs.genes,
                           rep(trs.mf.go,length(trs.genes)),
                           rep("ISS",length(trs.genes)))

colnames(trs.genes.df) <- colnames(ostta_v2_go_annotation)




## Annotation from massive excel file
id.annotation <- read.table(file="manual_expansion/id_mapping_annotation.tsv",header=F,as.is=T,sep="\t")
head(id.annotation)
nrow(id.annotation)
id.annotation <- subset(id.annotation, V2 != "")
head(id.annotation)
nrow(id.annotation)

df.accum <- data.frame(GID=NULL,GO=NULL,EVIDENCE=NULL)
for(i in 1:nrow(id.annotation))
{
 print(i)
 go.terms <- strsplit(id.annotation[i,2], split="; ")[[1]]
 df.accum <- rbind(df.accum, 
                   data.frame(GID=rep(id.annotation[i,1],length(go.terms)),
                              GO=go.terms,
                              EVIDENCE=rep("ISS",length(go.terms))))
}

ostta_v3_go_annotation <- rbind(ostta_v2_go_annotation,chromatin.genes.df,lipid.genes.df,tfs.genes.df,trs.genes.df)#,chromatin.genes.df)
ostta_v4_go_annotation <- rbind(ostta_v3_go_annotation, df.accum)
...
# Save the ostta_v2_go_annotation dataframe to a TSV file without quotes
write.table(ostta_v4_go_annotation, "./annotation/ostta_v4_go_annotation.tsv", sep = "\t", quote = FALSE, row.names = FALSE)

# Is there any improvement ostta_v2_go_annotation compared to version 1?
ostta.annoation.v4 <- read.table(file="./annotation/ostta_v4_go_annotation.tsv",header=T,sep="\t")

head(ostta.annoation.v4)
length(unique(ostta.annoation.v4$GID))
length(unique(ostta.annoation.v4$GID))/7668 #we see that the annotation has improved around 4%
## 74.5% functionally annotated with a GO term


## Version v5
blast2go <- read.table(file="blast2go/blast2go.tsv",header=T,as.is=T,sep="\t")
head(blast2go)
nrow(blast2go)
blast2go <- subset(blast2go, GO.IDs != "")
head(blast2go)
nrow(blast2go)

i <- 1
df.b2g.accum <- data.frame(GID=NULL,GO=NULL,EVIDENCE=NULL)
for(i in 1:nrow(blast2go))
{
 print(i)
 go.terms <- substr(x = strsplit(blast2go[i,2], split="; ")[[1]],
                    start = 3,stop = 12)
 df.b2g.accum <- rbind(df.b2g.accum, 
                   data.frame(GID=rep(blast2go[i,1],length(go.terms)),
                              GO=go.terms,
                              EVIDENCE=rep("ISS",length(go.terms))))
}

ostta_v5_go_annotation <- rbind(ostta_v4_go_annotation, df.b2g.accum)
...
# Save the ostta_v2_go_annotation dataframe to a TSV file without quotes
write.table(ostta_v5_go_annotation, "./annotation/ostta_v5_go_annotation.tsv", sep = "\t", quote = FALSE, row.names = FALSE)

# Is there any improvement ostta_v2_go_annotation compared to version 1?
ostta.annoation.v5 <- read.table(file="./annotation/ostta_v5_go_annotation.tsv",header=T,sep="\t")

head(ostta.annoation.v5)
...
## 87.5% functionally annotated with a GO term


## Version v6
go.annotation.2025 <- read.table(file="go_annotation_updated_2025.tsv",header=T,sep="\t")
head(go.annotation.2025)

df.go.2025.accum <- data.frame(GID=NULL,GO=NULL,EVIDENCE=NULL)
for(i in 1:nrow(go.annotation.2025))
{
  print(i)
  go.terms <- strsplit(go.annotation.2025[i,2], split="; ")[[1]]

  df.go.2025.accum <- rbind(df.go.2025.accum, 
                        data.frame(GID=rep(go.annotation.2025[i,1],length(go.terms)),
                                   GO=go.terms,
                                   EVIDENCE=rep("ISS",length(go.terms))))
}

head(df.go.2025.accum)

ostta_v6_go_annotation <- rbind(ostta_v5_go_annotation, df.go.2025.accum)

# Get the order of rows based on the first column; first, we need to create the vector
order <- order(ostta_v6_go_annotation[, 1])
# Reorder the rows of the dataframe according to the order vector 
ostta_v6_go_annotation <- ostta_v6_go_annotation[order, ]
head(ostta_v6_go_annotation)
# Observe
# Identify duplicate rows in columns 1 and 2 (genes and GO)
duplicate_rows <- duplicated(ostta_v6_go_annotation[, c(1, 2)]) | duplicated(ostta_v6_go_annotation[, c(1, 2)], fromLast = TRUE)
# Select only the duplicate rows
duplicate_data <- ostta_v5_go_annotation[duplicate_rows, ]
# View the result
print(duplicate_data)
...
# Remove
# Remove duplicate rows in columns 1 and 2 (genes and GO), leaving one copy
nrow(ostta_v6_go_annotation)
ostta_v6_go_annotation <- unique(ostta_v6_go_annotation[, c(1, 2, 3)])
nrow(ostta_v6_go_annotation)
# Print the result (optional)
print(ostta_v5_go_annotation)

# Save the ostta_v2_go_annotation dataframe to a TSV file without quotes
write.table(ostta_v6_go_annotation, "./annotation/ostta_v6_go_annotation.tsv", sep = "\t", quote = FALSE, row.names = FALSE)

# Is there any improvement ostta_v2_go_annotation compared to version 1?
ostta.annoation.v6 <- read.table(file="./annotation/ostta_v6_go_annotation.tsv",header=T,sep="\t")

head(ostta.annoation.v6)
length(unique(ostta.annoation.v6$GID))
length(unique(ostta.annoation.v6$GID))/7668 #we see that the annotation has improved around 4%
## 88.3% functionally annotated with a GO term
...
makeOrgPackage(go=ostta.annoation.v6,
               version = "6.1",
